...
typedef enum operationType {
    ...
    OP_LE, // операция "<"
    OP_LEE, // операция "<="
    OP_LIKE, // операция "like"/"@@" (поиск подстроки)
    OP_AND, // операция "&"
    OP_OR, // операция "|"
    OP_NOT, // операция "!"
} operationType;
...
struct condition {
    ...
    union {
        element* el1; // первый элемент для сравнения (если операция - НЕ логическая)
        condition* cond1; // первое условие (если операция - логическая)
    };
    union {
        element* el2; // второй элемент для сравнения (если операция - НЕ логическая)
